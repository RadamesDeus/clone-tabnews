...
function Status({ tipo }) {
  ...
  if (!isLoading) {
    let loading = "teste";
    const { version, max_connections, opened_connections } = data.dependencies.database;
    switch (tipo) {
      case "update_at":
        loading = <div>última atualização: {new Date(data.update_at).toLocaleString("pt-BR")}</div>
        break;
      case "version":
